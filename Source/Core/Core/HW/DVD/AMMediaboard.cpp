...
static constexpr u8* GetSafePtr(std::span<u8> buffer, u32 buffer_base, u32 offset, u32 length)
{
  ...
  {
    const u32 off = offset - buffer_base;
                                      ...
                                      short requested_events)
{
  ...
  for (auto& pfd : pfds)
  {
    if (!guest_fds.IsFdSet(GuestSocket(index++)))
      continue;
                                       ...
                                       short returned_events)
{
  ...
  {
    if ((fd.revents & returned_events) == returned_events)
      continue;
...
static void AMMBCommandSelect(u32 parameter_offset, u32 network_buffer_base)
{
  ...
  // Populate a vector with all the corresponding host sockets less-than `nfds`.
  // Some of these may not actually be in the GuestFdSet, but INVALID_SOCKET are erase_if'd below.
  std::vector<WSAPOLLFD> pollfds(nfds);
  for (u32 guest_fd = FIRST_VALID_FD; guest_fd < nfds; ++guest_fd)
    pollfds[guest_fd].fd = GetHostSocket(GuestSocket(guest_fd));

  // TODO: There may be some edges cases where
